...
import (
	...
	"github.com/deepjyoti-sarmah/go-boilerplate/internal/middleware"
	"github.com/deepjyoti-sarmah/go-boilerplate/internal/server"
	"github.com/labstack/echo/v4"
)

type HeahtHander struct {
	Handler
}

func NewHealthHandler(s *server.Server) *HeahtHander {
	return &HeahtHander{
		Handler: NewHandler(s),
	}
}

func (h *HeahtHander) CheckHealth(c echo.Context) error {
	start := time.Now()
	logger := middleware.GetLogger(c).
		With().Str("operation", "health_check").
		Logger()
